...
from app.src.crud.get_user_by_email import get_user_by_id
...
def get_login_logs_by_user(db: Session, user_id: UUID) -> List[LoginLogs]:
  ...
  return (
    db.query(LoginLogs)
    .filter(LoginLogs.member_id == user.member_id)
    .order_by(LoginLogs.login_at.desc())
    .all()
  )


def get_login_attempt_count(db: Session, member_id: UUID) -> int:
  last_success = (
    db.query(LoginLogs)
    .filter(LoginLogs.member_id == member_id, LoginLogs.status == "success")
    .order_by(LoginLogs.login_at.desc())
    .first()
  )
  if not last_success:
    return (
      db.query(LoginLogs)
      .filter(LoginLogs.member_id == member_id, LoginLogs.status == "failure")
      .count()
    )
  return (
    db.query(LoginLogs)
    .filter(
      LoginLogs.member_id == member_id,
